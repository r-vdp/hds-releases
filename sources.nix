# this file is autogenerated by .github/workflows/build-hds.yml
{
  nixpkgs = builtins.fetchTarball {
    url = "https://github.com/nixos/nixpkgs/archive/9f2ee8c91ac42da3ae6c6a1d21555f283458247e.tar.gz";
    sha256 = "0imgfxzq7d7l6fcgnzzjvv6ch560svcm8s8bx8vqyvf60w24ma1d";
  };
  x86_64-linux.holo-dev-server-bin = builtins.fetchTarball {
    url = "https://github.com/r-vdp/hds-releases/releases/download/2024-02-19-231414/holo-dev-server-x86_64-linux.tar.gz";
    sha256 = "0igw676g14cacg6gawvpqxcx90q7vnfjr38iimsbyqvasmavg5r5";
  };
  aarch64-linux.holo-dev-server-bin = builtins.fetchTarball {
    url = "https://github.com/r-vdp/hds-releases/releases/download/2024-02-19-231414/holo-dev-server-aarch64-linux.tar.gz";
    sha256 = "0v4gw7pgqqi8qcdd0246aqddnsvw1f2hrdygwhpnqdlq7y18w55g";
  };
}
